...
{
	if (bh->b_lock)
		printk("hd: buffer multiply locked\n");
	bh->b_lock=1;
}
...
{
	if (!bh->b_lock)
		printk("hd: free buffer being unlocked\n");
	bh->b_lock=0;
	wake_up(&bh->b_wait);
}
...
for (drive = 0; drive<NR_HD ; drive++) {
	...
	if (start_buffer->b_data[510] != 0x55 || (unsigned char)
		...
		printk("\nBad partition table at drive %d\n", drive);
	}
	...
	printk("Partition%s ok.\n\r",(NR_HD>1)?"s are":" is");    /*!if hard disk count is above one then add "s" else don't add anything */
	mount_root();
	return (0);
}
